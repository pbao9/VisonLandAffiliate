...
$(document).ready(function () {
    $("form").submit(function () {
        $(this).prepend(
            '<div style="position: absolute;width: 100%;height: 100%;background: #ffffff91;z-index: 10;"></div>'
        );
        $(this).find("button[type='submit']").css("opacity", "0.5");
        $(this)
            .find("button[type='submit']")
            .html(
                '<span class="spinner-grow spinner-grow-sm"></span> Đang xử lý..'
            );
    });
});
...
$(document).on("click", ".open-modal-resend-payment", function () {
    var form = $("#modalFormResend"),
        action = $(this).data("route");
    form.attr("action", action);
});
...
$(document).on("click", ".open-modal-payment", function () {
    ...
    $("#modal-title").text(
        type === "direct"
            ? "Cập nhật hoa hồng trực tiếp"
            : "Cập nhật hoa hồng gián tiếp"
    );
    $(".amount_remaining").text(
        "Tiền chưa thanh toán: " + number_format(amount) + "đ"
    );
...
$(document).ready(function () {
    $(".priceFormat").on("keyup", function () {
        ...
        const numericValue = unformattedValue.replace(/[^\d]/g, "");
        ...
        $(this).data("unformatted-value", numericValue);

        $(".hiddenCurrency").val(numericValue);
    });
    $(".priceFormat").on("input", function () {
        var inputValue = $(this).val();
        $(".hiddenCurrency").val(
            inputValue.replace(/[,|.]/g, "").replace(/[^\d]/g, "")
        );
    });
});
