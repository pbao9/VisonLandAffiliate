...
class UseDefaultValetDbCredentialsSolution implements RunnableSolution
{
    ...
    {
        return 'Pressing the button will change `DB_USER` and `DB_PASSWORD` in your `.env_backup` file.';
    }
    ...
    {
        if (! file_exists(base_path('.env_backup'))) {
            return;
        }
    ...
    {
        return 'You seem to be using Valet, but the .env_backup file does not contain the right default database credentials.';
    }
}
